% This function group-mean-nonezero-projects an input stack according to given
...
%%ELiiiiiii, 20240421
function projResult = groupMean_nz(inputStack, groupSize, cropFlag, projDim)
%%
if nargin < 4
    projDim = 3;
end
...
for projCount = 1:projResultSize3
    projResult(:,:,projCount) = squeeze(mean_nz(inputStack(:,:,(projCount-1)*groupSize+1:min(projCount*groupSize,size3)),projDim));
end