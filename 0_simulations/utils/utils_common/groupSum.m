% This function group-sum-projects an input stack according to given
...
%%ELiiiiiii, 20210819
function projResult = groupSum(inputStack, groupSize, cropFlag, projDim)
%%
if nargin < 4
    projDim = 3;
end
...
for projCount = 1:projResultSize3
    projResult(:,:,projCount) = squeeze(sum(inputStack(:,:,(projCount-1)*groupSize+1:min(projCount*groupSize,size3)),projDim));
end